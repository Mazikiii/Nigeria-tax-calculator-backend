...
use entities::{CategoryRule, ParsedStatment, RawStatment, TransactionRole, LexiconFile};
...
// the hashmap that has keywords another that has patterns stored and a charge of type regex
    ...
    pattern_container: Hashmap<String, PatternRule>,
    charge: Regex,
}
...
impl TransactionCategorizer{
    ...
    fn new() -> self{
        ...
        let json_to_string = fs::read_to_String(src/domain/lexicon.json).expect("Failed to read Json");
        ...
        let arranged_string: LexiconFile = serde_json::from_str(json_to_string).expect("Failed to arrange string");
    ...
    fn extract_charge(&self, narration: &str) -> Decimal{
        ...
        self.charge.captures(narration)
    ...
    pub fn analyze_raw_statment(&self, raw: RawStatement) -> ParsedTransaction{
        ...
        let narration_words: Hashset<String> = &raw.narration
            .iter()
            .to_upppercase().map(|s| s.to_uppercase ).collect()
